...
const DiariAnakScreen: React.FC<DiariAnakScreenProps> = ({ onBack, onNavigate }) => {
  ...
  const loadChildren = async () => {
    try {
      ...
      // Don't show error alert for children, just use empty array
      setChildren([]);
    } finally {
  ...
  const handleEditNote = (note: StickyNote) => {
    ...
    // Set selected child if note has child_id
    if (note.child_id) {
  ...
  const pickImage = async () => {
    try {
      ...
      if (!result.canceled && result.assets && result.assets.length > 0) {
        ...
        setNewNote({ ...newNote, image: imageUri });
      }
    } catch (error) {
  ...
  const renderAddForm = () => (
    <View style={styles.addFormContainer}>
      ...
      <View style={styles.childSelectionContainer}>
        <Text style={styles.childSelectionLabel}>Pilih Anak (Opsional)</Text>
        <TouchableOpacity 
        ...
        >
          <Text style={[
            ...
            !selectedChild && styles.childDropdownPlaceholder
          ]}>
            {selectedChild ? selectedChild.name : 'Pilih anak untuk catatan ini'}
          </Text>
          <Ionicons 
            name={showChildDropdown ? "chevron-up" : "chevron-down"} 
            size={20} 
            color="#666666" 
          />
        </TouchableOpacity>
        ...
        {selectedChild && (
          <TouchableOpacity 
            style={styles.selectedChildContainer}
        ...
        {showChildDropdown && (
          <View style={styles.childDropdownList}>
            {children.map((child) => (
              <TouchableOpacity
                key={child.id}
            ...
            {children.length === 0 && (
              <Text style={styles.noChildrenText}>Belum ada data anak</Text>
            )}
          </View>
        )}
      </View>
      ...
      <View style={styles.photoSection}>
        ...
        {newNote.image && newNote.image.trim() !== '' && newNote.image !== 'null' ? (
          <View style={styles.selectedImageContainer}>
            <Image 
              source={{ 
                uri: newNote.image.startsWith('http') || newNote.image.startsWith('file://') || newNote.image.startsWith('data:') 
                  ? newNote.image 
                  : getImageUrl(newNote.image) || newNote.image
              }} 
