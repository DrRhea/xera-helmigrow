...
const NutritionScreen: React.FC<NutritionScreenProps> = ({ onBack, childData }) => {
  ...
  const [refreshedChildData, setRefreshedChildData] = useState<Child | null>(null);
  ...
  const refreshChildData = async () => {
    try {
      ...
      if (Array.isArray(children) && children.length > 0) {
        ...
        const updatedChild = children.find((child: Child) => child.id === childData.id);
        if (updatedChild) {
          setRefreshedChildData(updatedChild);
        }
      }
    } catch (error) {
  ...
  // Refresh child data when component mounts
  useEffect(() => {
    refreshChildData();
  }, []);

  // Use refreshed data if available, otherwise use original data
  const displayChildData = refreshedChildData || childData;
  ...
  // WHO Standards data per month (0-60 months) - Updated according to BB/U table
  const whoStandards = {
    boys: {
      weight: [
  ...
  const handleCalculateNutrition = () => {
    ...
    const ageInMonths = calculateAgeInMonths(displayChildData.birth_date);
    ...
    // Validasi usia
    if (ageInMonths === -1) {
      Alert.alert('Error', 'Data tanggal lahir tidak valid. Silakan periksa data anak.');
      return;
    }
    ...
    const result = calculateNutritionStatus(weight, height, ageInMonths, displayChildData.gender);
  ...
  const getAgeDisplay = () => {
    if (ageInMonths === -1) {
      return 'Usia tidak valid - periksa data tanggal lahir';
    } else if (ageInMonths === 0) {
      return 'Usia tidak valid - tanggal lahir tidak valid';
  ...
  if (!displayChildData.birth_date || displayChildData.birth_date.trim() === '' || ageInMonths === -1) {
    return (
      <View style={styles.container}>
        ...
        <View style={styles.errorContainer}>
          <Text style={styles.errorText}>
            {!displayChildData.birth_date || displayChildData.birth_date.trim() === '' 
              ? 'Data tanggal lahir anak tidak valid atau kosong.'
              : 'Data tanggal lahir anak tidak dapat diproses. Periksa format tanggal.'
            }
          </Text>
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.childInfoCard}>
          ...
          <View style={styles.childInfo}>
            <Text style={styles.childName}>{displayChildData.name}</Text>
            <Text style={styles.childDetails}>
              {getAgeDisplay()} • {displayChildData.gender}
            </Text>
          </View>
        </View>
